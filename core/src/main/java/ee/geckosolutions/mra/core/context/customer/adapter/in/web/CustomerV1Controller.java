...
public class CustomerV1Controller {
    ...
    public ResponseEntity<CustomerV1> get(@PathVariable UUID id) {
        CustomerV1 customerV1 = this.customerV1WebMapper.toCustomerV1(this.customerApplicationService.getById(id));
    ...
    public ResponseEntity<CustomerV1> insert(@Valid @RequestBody NewCustomerV1 newCustomerV1) {
        Customer customer = switch (newCustomerV1.getType()) {
            case PRIVATE -> this.customerApplicationService.createPersonCustomer(
                    newCustomerV1.getFirstName(),
                    newCustomerV1.getLastName(),
                    newCustomerV1.getPersonalIdentificationCode());
            case COMPANY -> this.customerApplicationService.createLegalEntityCustomer(
                    newCustomerV1.getName(), newCustomerV1.getRegistrationCode());
        };
        CustomerV1 customerV1 = this.customerV1WebMapper.toCustomerV1(customer);
        return ResponseEntity.ok(customerV1);
    }
